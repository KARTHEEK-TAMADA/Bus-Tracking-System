...
export default function DriverDashboard({ user }) {
  ...
  const startTracking = () => {
    ...
    if (!window.confirm("Are you sure you want to start sharing your live location with students?")) return;
    ...
    <div className="flex items-center justify-center min-h-[85vh] p-4 bg-slate-50">
      <div className="bg-white max-w-md w-full p-6 sm:p-8 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-slate-100 flex flex-col pt-10 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-[#1D3557] tracking-tight">Driver Screen</h2>
        ...
        {!isTracking ? (
          <div className="space-y-6 animate-[fadeIn_0.5s_ease-out]">
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Choose Your Bus</label>
              {buses.length > 0 ? (
                <select 
                ...
                >
                  <option value="" className="font-normal text-slate-500">-- Choose Bus --</option>
                  {buses.map(b => <option key={b.id} value={b.id}>{b.bus_number}</option>)}
                </select>
              ) : (
                <div className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl text-center text-slate-500 font-medium">
                  No buses for you right now. Ask admin.
                </div>
              )}
            </div>
            ...
            <button 
              ...
              className={`w-full ${buses.length === 0 ? 'bg-slate-300' : 'bg-[#1D3557] hover:bg-[#457B9D]'} text-white font-bold py-4 px-4 rounded-xl transition-all shadow-xl shadow-[#1D3557]/20 active:scale-95 text-lg flex justify-center items-center gap-2 group`}
            >
              Start Driving <span className="group-hover:translate-x-1 transition-transform">→</span>
            </button>
          </div>
        ) : (
          <div className="space-y-8 animate-[fadeIn_0.5s_ease-out] flex flex-col items-center">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-slate-500 font-medium">Students can see your bus now on the map.</p>
            </div>
            
            {location ? (
              <div className="w-full bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-row font-mono text-[11px] sm:text-xs text-slate-600 justify-between items-center text-center">
                <div className="flex flex-col flex-1"><span className="text-slate-400 font-sans uppercase font-bold text-[9px] sm:text-[10px]">Latitude</span>{location.lat.toFixed(6)}</div>
                <div className="w-px h-8 bg-slate-200 mx-1"></div>
                <div className="flex flex-col flex-1"><span className="text-slate-400 font-sans uppercase font-bold text-[9px] sm:text-[10px]">Longitude</span>{location.lng.toFixed(6)}</div>
                <div className="w-px h-8 bg-slate-200 mx-1"></div>
                <div className="flex flex-col flex-1"><span className="text-slate-400 font-sans uppercase font-bold text-[9px] sm:text-[10px]">Accuracy</span>±{location.accuracy?.toFixed(0) || 0}m</div>
              </div>
            ) : (
